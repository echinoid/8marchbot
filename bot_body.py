...
def select_user_data (user_id, id, field):
    if id == 'last':
        record_id = last_id_sql = 'SELECT max(' + field + ') FROM user_' + str(user_id)
    else: record_id = id
...
def step2(call):
    ...
    last_message_id = select_user_data(call.from_user.id, 'last', 'lastid')
    bot.edit_message_text(last_message_id, reply_markup= None)
